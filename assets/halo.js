(function ($) {
  ...
  win.off("resize.initMenuMobile").on("resize.initMenuMobile", function () {
    ...
    resizeTimerId = setTimeout(function () {
      ...
      if (
        (curWinWidth < 1200 && winWidth >= 1200) ||
        (curWinWidth >= 1200 && winWidth < 1200)
      ) {
        ella.showHideMenuMobile();
        ella.initToggleMuiltiLangCurrency();
        ella.addTextMuiltiOptionActive(
          $("#lang-switcher"),
  ...
  var ella = {
    ...
    closeHeaderTop: function () {
      ...
      if (closeHeaderTopElm.length && closeHeaderTopElm.is(":visible")) {
        ...
        closeHeaderTopElm
          .off("click.closeHeaderTop")
          .on("click.closeHeaderTop", function (e) {
            e.preventDefault();
            e.stopPropagation();
    ...
    showHideMenuMobile: function () {
      if (iconNav.length && iconNav.is(":visible")) {
        iconNav
          .off("click.showMenuMobile")
          .on("click.showMenuMobile", function (e) {
            e.preventDefault();
            e.stopPropagation();
    ...
    closeAllOnMobile: function () {
      body
        .off("click.close", wrapperOverlaySlt)
        .on("click.close", wrapperOverlaySlt, function (e) {
          e.preventDefault();
          e.stopPropagation();
    ...
    initToggleMuiltiLangCurrency: function () {
      ...
      if (dropdownLabel.length && dropdownLabel.is(":visible")) {
        dropdownLabel
          ...
          .on("click.toggleMuiltiOption", function (e) {
            e.preventDefault();
            e.stopPropagation();
    ...
    hideMuiltiLangCurrency: function () {
      doc
        ...
        .on("click.hideMuiltiLangCurrency", function (e) {
          ...
          if (
            !muiltiDropdown.is(e.target) &&
            !muiltiDropdown.has(e.target).length
          ) {
          ...
          infiniteScrollingLinkSlt
            ...
            .on("click.showMoreProduct", function (e) {
              ...
              if (!$(this).hasClass("disabled")) {
                ella.doAjaxInfiniteScrollingGetContentSection(
                  url,
                  limit,
                  page,
                  e,
    ...
    initSliderFeaturedProducts: function () {
      ...
      featuredProduct.each(function () {
        ...
        if (productGrid.not(".slick-initialized")) {
          productGrid
            .parent()
            .siblings(".widget-title")
            .append(`<div class="slider-arrow-container"></div>`);
          ...
          productGrid.slick({
            ...
            appendArrows: productGrid
              .parent()
              .siblings(".widget-title")
              .find(".slider-arrow-container"),
            nextArrow:
    ...
    initDropdownCustomerTranslate: function (iconUser, sltShowUser) {
      if (iconUser.length && iconUser.is(":visible")) {
        iconUser
          ...
          .on("click.dropdownCustomerMobile", function (e) {
            e.preventDefault();
            e.stopPropagation();
    ...
    closeTranslate: function (closeElm, classRemove) {
      if ($(closeElm).length) {
        body
          .off("click.closeCustomer", closeElm)
          .on("click.closeCustomer", closeElm, function (e) {
            e.preventDefault();
            e.stopPropagation();
    ...
    closeDropdownCustomerTranslate: function () {
      ella.closeTranslate(
        "#dropdown-customer .close-customer",
        "customer-show"
      );
    },
    ...
    initDropdownCartMobile: function () {
      ...
      cartIcon
        ...
        .on("click.initDropdownCartMobile", function (e) {
          e.preventDefault();
          e.stopPropagation();
    ...
    addEventShowOptions: function () {
      ...
      doc
        .off("click.showOptions", optionsIconSlt)
        .on("click.showOptions", optionsIconSlt, function (e) {
          e.preventDefault();
          e.stopPropagation();
      ...
      ella.closeTranslate(
        ".lang-currency-groups .close-option",
        "options-show"
      );
    },
    ...
    doUpdateDropdownCart: function (cart) {
      ...
      dropdownCart
        .find(".summary .price")
        .html(Shopify.formatMoney(cart.total_price, window.money_format));
      $("#sidebar-cart")
      ...
      if (cart.item_count > 0) {
        for (var i = 0; i < cart.items.length; i++) {
          ...
          item = item.replace(
            /\{TITLE\}/g,
            ella.translateText(cart.items[i].product_title)
          );
          item = item.replace(
            /\{VARIANT\}/g,
          ...
          item = item.replace(
            /\{IMAGE\}/g,
            Shopify.resizeImage(cart.items[i].image, "160x")
          );
          if (cart.cart_level_discount_applications.length > 0) {
            item = item.replace(
              /\{PRICE\}/g,
              Shopify.formatMoney(
                cart.items[i].discounted_price,
                window.money_format
              )
            );
          } else {
            item = item.replace(
              /\{PRICE\}/g,
              Shopify.formatMoney(cart.items[i].price, window.money_format)
            );
          }
    ...
    translateText: function (str) {
      if (!window.multi_lang || str.indexOf("|") < 0) return str;

      if (window.multi_lang) {
        var textArr = str.split("|");

        if (translator.isLang2()) return textArr[1];
        return textArr[0];
      }
    },
    ...
    initColorSwatchGrid: function () {
      ...
      body
        ...
        .on("click.toggleClass", itemSwatchSlt, function () {
          var self = $(this),
            productItemElm = self.closest(".grid-item"),
            sidebarWidgetProduct = productItemElm.closest(
      ...
      $.ajax({
        ...
        error: function (xhr, text) {
          $(".ajax-error-message").text(
            $.parseJSON(xhr.responseText).description
          );
    ...
    doAjaxProductTabs: function (handle, loadingElm, curTabContent) {
      ...
      $.ajax({
        ...
        error: function (xhr, text) {
          loadingElm
            .text("Sorry, there are no products in this collection")
            .show();
        },
      });
    },
    ...
    initProductTabsSlider: function (slt) {
      slt.each(function () {
        ...
        if (
          productGrid.not(".slick-initialized") &&
          productGrid.find(".grid-item").length
        ) {
          productGrid.slick({
    ...
    closeEmailModalWindow: function (newsletterWrapper, expire) {
      ...
      var inputChecked = newsletterWrapper
        .find('input[name="dismiss"]')
        .prop("checked");
      if (
        inputChecked ||
        !newsletterWrapper.find('input[name="dismiss"]').length
      )
      ...
      if (window.newsletter_popup) {
        ...
        newsletterWrapper.on("click", function (e) {
          if (
            ...
            !modalContent.has(e.target).length
          ) {
            ella.closeEmailModalWindow(newsletterWrapper, expire);
          }
        });
    ...
    initOpenSidebar: function () {
      ...
      if (sidebarLabelElm.length) {
        body
          ...
          .on("click.openSidebar", sidebarLabelSlt, function () {
    ...
    initDropdownSubCategoriesAtSidebar: function () {
      ...
      body
        ...
        .on("click.toggleSubCategories", iconDropdownSlt, function (e) {
          e.stopPropagation();
      ...
      body
        ...
        .on("click.linktoCollection", linkDropdownSlt, function (e) {
          e.stopPropagation();
        });
    },
    ...
    setTextForSortbyFilter: function () {
      ...
      if (Shopify.queryParams.sort_by) {
        var sortBy = Shopify.queryParams.sort_by,
          sortByLinkActive = filterSortby.find(
            'span[data-href="' + sortBy + '"]'
          ),
    ...
    setTextForLimitedFilter: function () {
      ...
      if (filterLimited.length) {
        var limited = filterLimited.find("li.active span").data("value"),
          limitedActive = filterLimited.find(
            'span[data-value="' + limited + '"]'
          ),
    ...
    ajaxFilterSortby: function () {
      ...
      body
        .off("click.sortBy", sortbyFilterSlt)
        .on("click.sortBy", sortbyFilterSlt, function (e) {
          e.preventDefault();
          e.stopPropagation();
    ...
    ajaxFilterLimit: function () {
      ...
      body
        .off("click.sortBy", limitFilterSlt)
        .on("click.sortBy", limitFilterSlt, function (e) {
          e.preventDefault();
          e.stopPropagation();
    ...
    ajaxFilterClearAll: function () {
      ...
      body
        .off("click.clearAllTags", clearAllSlt)
        .on("click.clearAllTags", clearAllSlt, function (e) {
          e.preventDefault();
          e.stopPropagation();
    ...
    initToggleWidgetTitleSidebarFilter: function () {
      ...
      body
        ...
        .on("click.toggleWidgetContent", widgetTitleSlt, function () {
          $(this).toggleClass("open");
          $(this).next().slideToggle();
        });

      var widgetTitleSltCollNoSidebar =
    ...
    initInfiniteScrolling: function () {
      ...
      if (infiniteScrolling.length) {
        body
          .off("click.initInfiniteScrolling", infiniteScrollingLinkSlt)
          .on(
    ...
    ajaxInfiniteScrollingMapData: function (data) {
      ...
      if (newProductColl.length) {
        ...
        if ($(".collection-template .product-collection[data-layout]").length) {
          ella.ellaTimeout = setTimeout(function () {
            currentProductColl
              .isotope("appended", newProductItem)
              .isotope("layout");
          }, 700);
        }
        ...
        default:
          switch (col) {
            case 2:
              if (gridItem.hasClass("grid-item-mansory")) {
                products
                  .removeClass(gridClass)
                  .addClass("products-grid grid-2");
              } else {
            ...
            case 5:
              if (gridItem.hasClass("grid-item-mansory")) {
                products
                  .removeClass(gridClass)
                  .addClass("products-grid grid-5");
              } else {
                products.removeClass("products-list").addClass("products-grid");
              }
              gridItem
                .removeClass(strClass)
                .addClass("col-6 col-md-4 col-lg-3 col5");
              break;
          }
      }
    },
    ...
    changeQuantityAddToCart: function () {
      ...
      doc.on("click", buttonSlt, function (e) {
        ...
        var self = $(this),
          input =
            self.siblings('input[name="quantity"]').length > 0
              ? self.siblings('input[name="quantity"]')
              : self.siblings('input[name="group_quantity"]');
    ...
    initAddToCart: function () {
      ...
      doc
        .off("click.addToCart", btnAddToCartSlt)
        .on("click.addToCart", btnAddToCartSlt, function (e) {
          e.preventDefault();
          e.stopPropagation();
          var self = $(this);
          ...
          if (self.attr("disabled") !== "disabled") {
            ...
            if (productItem.length < 1) {
              var sectionsProduct = self.closest(
            ...
            var form = productItem.find("form"),
              handle =
                productItem
                  .find(".product-grid-image")
                  .data("collections-related") ||
                sectionsProduct.data("collections-related");
    ...
    initGroupedAddToCart: function () {
      ...
      doc
        .off("click.GroupedAddToCart", btnAddToCartSlt)
        .on("click.GroupedAddToCart", btnAddToCartSlt, function (e) {
          e.preventDefault();
          e.stopPropagation();
          ...
          if (self.attr("disabled") !== "disabled") {
            ...
            groupedProduct.each(function () {
              (variantId = $(this).find("input[type=hidden]").attr("value")),
                (quantity = parseInt(
                  $(this).find("input[name=group_quantity]").val()
                ));
    ...
    changeVariantSelectOption: function () {
      ...
      doc.on("change", selectSlt, function () {
        ...
        parent
          ...
          .html(Shopify.formatMoney(dataPrice, window.money_format));
    ...
    initChangeQuantityButtonEvent: function () {
      ...
      doc
        ...
        .on("click.updateCart", buttonSlt, function (e) {
          e.preventDefault();
          e.stopPropagation();
      ...
      doc.on("change", swatchSlt, function () {
        ...
        if (
          ...
          if (productShop.closest(".product-slider").length) {
            var productImgs = productShop
                .closest(".product-slider")
                .find("[data-moreview-product-slider]"),
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          setTimeout(function () {
            ...
            ella.initProductMoreview(
              $("[data-more-view-product-qv] .product-img-box")
            );
    ...
    openSearchForm: function () {
      ...
      doc
        ...
        .on("click.toggleSearch", iconSearchSlt, function (e) {
          e.preventDefault();
          e.stopPropagation();
    ...
    handleScrollDown: function () {
      ...
      iconSrollDown.each(function () {
        ...
        doc
          .off("click.handleScrollDown", iconSrollDownSlt)
          .on("click.handleScrollDown", iconSrollDownSlt, function (e) {
            e.preventDefault();
      ...
      if (blockSticky.length) {
        var showHideVariantsOptions = function () {
          ...
          doc
            ...
            .on("click.hideVariantsOption", function (e) {
              if (
                !sltVariantActive.is(e.target) &&
                sltVariantActive.has(e.target).length === 0
              ) {
        ...
        var handleActiveVariant = function () {
          variantElm.off("click.activeVar").on("click.activeVar", function (e) {
            ...
            var swatchNameValue = $(
              '#add-to-cart-form [data-value-sticky="' + value + '"]'
            );
        ...
        var activeVariantSelected = function () {
          var optionSelected = $("#product-selectors option:selected"),
            ...
            stickyActive = blockSticky.find(
              '.pr-swatch[data-value="' + value + '"]'
            ),
          ...
          $(".swatch .swatch-element").each(function (e) {
            ...
            $('.swatch input.text[data-value="' + idVariant + '"]').appendTo(
              $(this)
            );
          });
      ...
      jQuery.getJSON(
        ...
        function (product) {
          ...
          productHTML +=
            ...
            '">';
          productHTML +=
            '<div class="column col-img"><div class="product-image">';
          productHTML +=
            '<a href="' +
            product.url +
        ...
        .on("click.wl-pagging", "li a", function (e) {
          ...
          if (isPrev) {
            var curPage = parseInt(
              $(this).parent().siblings(".active").children().data("page")
            );
    ...
    doAddOrRemoveWishlish: function () {
      ...
      doc
        .off("click.addOrRemoveWishlist", iconWishListsSlt)
        .on("click.addOrRemoveWishlist", iconWishListsSlt, function (e) {
          e.preventDefault();
    ...
    initBundleProducts: function () {
        ...
        if (bundlePrice.length > 0) {
          ...
          if (discountText.length > 0) {
            discountText.each(function () {
              $(this)
                .text($(this).text().replace("[discount]", discountRate))
                .parent()
                .show();
            });
          }
      ...
      var bundleSlider = function () {
        if (
          bundleImagesSlide.length &&
          bundleImagesSlide.not(".slick-initialized")
        ) {
      ...
      var toggleVariantOptions = function () {
        body
          .off("click.toggleVariantOptions", btnToggleOptionsSlt)
          .on("click.toggleVariantOptions", btnToggleOptionsSlt, function (e) {
            e.preventDefault();
      ...
      var handleCheckedProduct = function () {
        ...
        body
          .off("click.checkedProduct", bundleCheckbox)
          .on("click.checkedProduct", bundleCheckbox, function (e) {
            e.preventDefault();

            var self = $(this),
              ipt = self.prev(),
              dataId = self
                .closest(".fbt-product-item")
                .data("bundle-product-id");

            if (!ipt.prop("checked")) {
              ipt.prop("checked", true);
              $('[data-bundle-product-id="' + dataId + '"]').addClass(
                "isChecked"
              );
            } else {
              ipt.prop("checked", false);

              $('[data-bundle-product-id="' + dataId + '"]').removeClass(
                "isChecked"
              );
            }
            updateTotalPrice();
        ...
        bundleProItem.each(function () {
          ...
          if (price) {
            ...
            if (bundlePrice.length > 0 && oldPrice.length > 0) {
              oldPrice.html(
                Shopify.formatMoney(totalPrice, window.money_format)
              );
              bundlePrice.html(
                Shopify.formatMoney(
            ...
            $(".products-grouped-action .total .price").html(
              Shopify.formatMoney(totalPrice, window.money_format)
            );
          }
        });
      ...
      var disableSoldoutSwatchBundle = function (productItem) {
        ...
        var selectedSwatchOpt1 = productItem
          .find('[data-option-idx="0"]')
          .find("input:checked")
          .val();
        var selectedSwatchOpt2 = productItem
          .find('[data-option-idx="1"]')
          .find("input:checked")
          .val();
        var selectedSwatchOpt3 = productItem
          ...
          switch (optionIndex) {
            ...
            case 1:
              ...
              swatch.each(function () {
                ...
                var opt1Soldout = variantList.find(function (variant) {
                  return (
                    variant.option1 == selectedSwatchOpt1 &&
            ...
            case 2:
              ...
              swatch.each(function () {
                ...
                var opt1Soldout = variantList.find(function (variant) {
                  return (
                    variant.option1 == selectedSwatchOpt1 &&
                    variant.option2 == selectedSwatchOpt2 &&
      ...
      var changeSwatchProductBundle = function () {
        ...
        doc
          .off("change.changeBundleSwatch", swatchSlt)
          .on("change.changeBundleSwatch", swatchSlt, function (e) {
            var self = $(this);
            if (self.prop("checked")) {
              ...
              switch (optionIdx) {
                case 0:
                  ...
                  if (availableVariants != undefined) {
                    ...
                    var altAvailableVariants = variantList.find(function (
                      variant
                    ) {
              ...
              productItem
                .find("input[name=id][type=hidden]")
                .val(selectedVariant.id);
              ...
              var newImage = productInput
                .find("option:selected", this)
                .attr("data-image");
              if (newImage != undefined) {
                var productImage = $(
                  '.fbt-image-item[data-bundle-product-id="' + productId + '"]'
                ).find("img");
      ...
      var initBundleAddToCart = function () {
        doc
          .off("click.bundleAddToCart", btnAddToCartSlt)
          .on("click.bundleAddToCart", btnAddToCartSlt, function (e) {
            e.preventDefault();
            ...
            var self = $(this),
              ...
              bundleProduct = form.find(
                "[data-grouped-product-item].isChecked"
              ),
        ...
        $.ajax({
          ...
          success: function (response) {
            window.location.href =
              "/discount/" + discount_code + "?redirect=/cart";
    ...
    checkBundleProducts: function () {
      ...
      if (discount_code != "" && discount_code != null) {
        ...
        if (mainProduct != "") getShoppingCart();
      }
      ...
      function getShoppingCart() {
        if ($("ul.cart-list li").length > 0) {
          ...
          $("ul.cart-list li").each(function (i, el) {
            ...
            if (cart.indexOf(product_id) == -1) cart.push(product_id);
          });
      ...
      function checkProductInCart(cart, collection) {
        ...
        if (i != collection.length) remove_discount();
      }
    ...
    productRecomendation: function () {
      ...
      $container
        .parent()
        .load(productRecommendationsUrlAndContainerClass, function () {
          if (template != "") {
